...
* Option to do a custom animation and sound on a hit, miss, critical miss and/or critical success. Requires that the module https://foundryvtt.com/packages/autoanimations be installed (along with it's dependencies). For the default miss animation install the module https://foundryvtt.com/packages/JB2A_DnD5e. For the default miss sound install the module https://foundryvtt.com/packages/soundfxlibrary. Or replace with your own animation and/or sound. The name of the sound file can instead be the name of a table to draw a random sound from, make a table with a text line for each sound that has the path to the sound, setting the table's dice formula to, say, 1d23 if the table has 23 entries. (If you want to use separate tables for each of critical miss/miss/hit/critical hit simply enter a different table name in the sound field for each category in the Workbench settings.) You can use this macro to output all the sound file paths from a playlist to the console: ```game.playlists.forEach(s=>s.data.sounds.forEach(x=>console.log(s.data.name + ":"+ x.data.name + ":" + x.data.path)))```
