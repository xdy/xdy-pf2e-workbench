...
import { isActuallyDamageRoll } from "../utils.ts";
import {
...
import { chatRenderSettings } from "../xdy-pf2e-workbench.ts";

function needsCollapsing(setting: string): boolean {
    return setting === "collapsedDefault" || setting === "nonCollapsedDefault";
}

export function renderChatMessageHTMLHook(message: ChatMessagePF2e, html: HTMLElement): void {
    handleDyingRecoveryRoll(message, chatRenderSettings.handleDyingRecoveryRoll);

    const isDamageRoll = isActuallyDamageRoll(message);

    if (isDamageRoll) {
        if (["expandedAll", "expandedNew", "expandedNewest"].includes(chatRenderSettings.expandDamageRolls)) {
            damageCardExpand(message, html, chatRenderSettings.expandDamageRolls);
        }

        if (chatRenderSettings.castPrivateSpell && systems.getFlag(message, "origin.type") === "spell") {
            hideSpellNameInDamageroll(message, html);
        }
    } else {
        if (needsCollapsing(chatRenderSettings.collapseItemContent)) {
            chatCardDescriptionCollapse(html);
        }

        const itemType = message.item?.type ?? "";
        if (
            needsCollapsing(chatRenderSettings.collapseItemAttackContent) &&
            ["weapon", "melee", "spell"].includes(itemType)
        ) {
            chatAttackCardDescriptionCollapse(html);
        }

        if (needsCollapsing(chatRenderSettings.collapseItemActionContent) && (itemType === "action" || !message.item)) {
            chatActionCardDescriptionCollapse(html);
        }
    }

    if (chatRenderSettings.heroPointRules !== "no") {
